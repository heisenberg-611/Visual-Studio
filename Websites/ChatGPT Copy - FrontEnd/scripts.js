...
this.changeMenu = function (curId, targetId) {
    ...
    // 却换左边菜单样式或动画
    targetMenuTop = $("#main-nav a").eq(menus.indexOf(targetId)+1).offset().top + 13;
...
this.onMenuClick = function (e) {
    // 点击logo时，不改变菜单项，而是跳转到腾达英文官网
    if("nav-logo" == e.target.className) {
        var tendaOfficail = "https://www.tendacn.com";
        window.open(tendaOfficail);
        return;
    }
...
init: function () {
    ...
    if (!staInfo.loading) {
        ...
        if(!(/*OEM_CONFIG_HAS_EXTENDER|true*/true/* OEMTAG */)) {
            $("#statusExtender").off("click");
        }
        /*OEM END*/
...
setImage: function (obj) {
    ...
    //宏控制无线抗干扰
    if (CONFIG_ANTI_INTERFERENCE == "y") {
        $("#wrl_anti_interference").removeClass("none");
    }
...
setWlStatus: function (obj) {
    ...
    $("#status_wl_double_txt, #status_wl_info_txt").addClass("none");
    //双频合一开启
    if (obj.doubleBand === "1") {
        $("#status_wl_double_txt").removeClass("none");
        if (obj.wl24gEn === "1") {
            $("#status_wl_double_txt .wl-name").html(toHtmlCode(obj.wl24gName)).attr("title", obj.wl24gName);
        } else {
            $("#status_wl_double_txt .wl-name").html(_("Disable"));
        }
    } else {
        $("#status_wl_info_txt").removeClass("none");
        if (obj.wl24gEn == "1") {
            $("#status_wl_info_txt .wl24g-name").html(toHtmlCode(obj.wl24gName)).attr("title", obj.wl24gName);
        } else {
            $("#status_wl_info_txt .wl24g-name").html(_("Disable"));
        }

        if (obj.wl5gEn == "1") {
            $("#status_wl_info_txt .wl5g-name").html(toHtmlCode(obj.wl5gName)).attr("title", obj.wl5gName);
        } else {
...
setValue: function (obj) {
    ...
    if (obj.wl_mode != "ap" || obj.wl_en != "1") {
        ...
        if (obj.wl_mode != "ap") {
            ...
            // showErrMsg("guest_save_msg", _("This function is not available if WIFI is off."), true);
            showErrMsg("guest_save_msg", _("This function is not available if WIFI is off."), true);
        }
    } else {
...
setValue: function (obj) {
    ...
    var prop,
        ipv6Arr = ["wanAddr"], //ipv6地址输入框
        i = 0;
    inputValue(obj);
    ipv6Info.data = obj;
    $("#IPv6En").val() === "1" ? $("#ipv6Cfg").removeClass("none") : $("#ipv6Cfg").addClass("none"); //IPv6开关
    if(obj.lanAddr) { //没有LAN地址时，隐藏LAN显示
        $("#lanAddr").parents(".control-group").removeClass("none");
    } else {
        $("#lanAddr").parents(".control-group").addClass("none");
    }
...
checkData: function () {
    ...
    if (wan_type == "Static") {
        if (gateWay == wanIpv6) {
            showErrMsg("msg-err", _("The default IPv6 gateway cannot be the duplicate with WAN IPv6 address"));
            return false;
        } 
        ...
        if (wanPreDNS == wanAltDNS) {
            showErrMsg("msg-err",_("The primary DNS server and secondary DNS server cannot be the same."));
            return false;
        }
        if (!ipv6Info.checkIpv6InSameSegment(wanIpv6, gateWay, wanPrefixLen, wanPrefixLen)) {
            if (geteWayHeader !== "FE80") {
                showErrMsg("msg-err", _("The gateway and the IP address must be in the same network segment."));
                return false;
            }
        }

        // if (ipv6Info.checkIpv6InSameSegment(wanIpv6, lanPrefix, wanPrefixLen, wanPrefixLen)) {
...
changeLanCfg: function () {
    ...
    if ($("#prefixDelegate")[0].checked) {
        ...
        $lanPrefix.parents(".control-group").addClass("none");

    } else {
        $lanPrefix.removeAttr("disabled");
        $lanPrefix.parents(".control-group").removeClass("none");
    }
},
